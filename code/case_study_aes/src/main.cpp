...
static bool compute_timeout(const Emulator& main_emulator,
                            fault_simulator::Configuration& ctx, u32 limit,
                            u32 grace_time) {
  Emulator timeout_emu(main_emulator);

  timeout_emu.add_before_fetch_hook(
      [&](Emulator& emu, u32 address, u32, void*) {
  ...
  ctx.faulting_context.emulation_timeout =
      timeout_emu.get_emulated_time() + grace_time;
  return true;
}
...
int main(int argc, char** argv) {
  ...
  struct AesDfaExploitabilityModel : ExploitabilityModel {
    AesDfaExploitabilityModel(fault_simulator::Configuration& config)
        : m_config(config), m_cnt{0} {};

    Decision evaluate(const Emulator& emu, const Context&,
                      u32 address) override {
  ...
  for (const auto& section : config.binary) {
    main_emulator.write_memory(section.offset, section.bytes.data(),
                               section.bytes.size());
  }
  main_emulator.write_register(Register::SP,
                               config.ram.offset + config.ram.size);
  ...
  if (config.faulting_context.emulation_timeout == 0) {
    ...
    if (!compute_timeout(main_emulator, config, limit, grace_time)) {
      std::cout << termcolor::bright_red << "ERROR:" << termcolor::reset
                << " execution without faults executes more than " << limit
                << " instructions." << std::endl;
      std::cout << "Please provide a timeout via --timeout" << std::endl;
      return -1;
    }

    std::cout << "no timeout set, simulation without any faults executes "
              << std::dec
              << config.faulting_context.emulation_timeout - grace_time
              << " instructions" << std::endl;
    std::cout << "setting timeout to "
              << config.faulting_context.emulation_timeout << " instructions"
              << std::endl;
  }

  {
    ...
    timeout_emu.add_before_fetch_hook([&](Emulator& emu, u32, u32, void*) {
      ...
      if (start_time == 0 &&
          std::memcmp(computed, aes_test::state_after_round7_mixcolumns, 16) ==
              0) {
      ...
      if (start_time != 0) {
        for (u32 i = 0; i < 11; ++i) {
          if (!seen[i] &&
              std::memcmp(computed, aes_test::state_before_sbox_in_round[i],
                          16) == 0) {
            std::cout << "round " << std::dec << i << " start at "
                      << emu.get_time() << std::endl;
            seen[i] = true;
          }
        }
      }
      ...
      if (end_time == 0 &&
          std::memcmp(computed, aes_test::state_before_sbox_in_round[10], 16) ==
              0) {
    ...
    std::cout << "advanced " << std::dec << start_time
              << " instructions until after mixcolumns of 7th round"
              << std::endl;
    std::cout << "faults are injected until " << std::dec << end_time
              << " instructions" << std::endl;
  }
  ...
  for (const auto& spec : fault_models::all_fault_models) {
    auto exploitable_faults = fault_simulator::find_exploitable_faults(
        main_emulator, config, {std::make_pair(spec, 1)});
  ...
  for (const auto& spec : fault_models::non_single_bit_fault_models) {
    auto exploitable_faults = fault_simulator::find_exploitable_faults(
        main_emulator, config, {std::make_pair(spec, 2)});
    total_faults += exploitable_faults.size();
  }
  ...
  std::cout << "All tested models combined: " << std::dec << total_faults
            << " exploitable faults" << std::endl;
