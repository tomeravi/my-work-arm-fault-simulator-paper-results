...
key_expansion(u8* round_key, const u8* Key) {
  unsigned i, j, k;
  u8 tempa[4];
  for (i = 0; i < 4; ++i) {
...
static void __attribute__((noinline))
cipher(state_t* state, const u8* round_keys) {
  u8 round = 0;
...
// self calling makes it easy to detect whether control-flow countermeasure
